...
class Money(discord.Cog):
    ...
    async def give(self, ctx, member: discord.Member, amount: int):
        ...
        if amount < 0:
            ...
            await ctx.respond(f"You sent {amount} moneys to {member.mention}.\nYou now have {self.moneys[ctx.author.id]} moneys.", ephemeral=True)
    ...
    async def slots(self, ctx: discord.ApplicationContext, bet: int):
        ...
        emojis = {
            ...
            # "🔔": {3: 20, 4: 100, 5: 300},
            # "⭐️": {3: 25, 4: 125, 5: 500},
            "7️⃣": {3: 50, 4: 250, 5: 1000}
        }

        def r():
            return random.choices(list(emojis.keys()), weights=[1])[0]

        def render(box):
            return (
                f"Spinning for {bet} moneys!\n"
        ...
        paylines = [[0,3,6,9,12],[0,3,6,9,13],[0,3,6,10,12],[0,3,6,10,13],[0,3,6,10,14],[0,3,7,9,12],[0,3,7,9,13],[0,3,7,10,12],[0,3,7,10,13],[0,3,7,10,14],[0,3,7,11,13],[0,3,7,11,14],[0,4,6,9,12],[0,4,6,9,13],[0,4,6,10,12],[0,4,6,10,13],[0,4,6,10,14],[0,4,7,9,12],[0,4,7,9,13],[0,4,7,10,12],[0,4,7,10,13],[0,4,7,10,14],[0,4,7,11,13],[0,4,7,11,14],[0,4,8,10,12],[0,4,8,10,13],[0,4,8,10,14],[0,4,8,11,13],[0,4,8,11,14],[1,3,6,9,12],[1,3,6,9,13],[1,3,6,10,12],[1,3,6,10,13],[1,3,6,10,14],[1,3,7,9,12],[1,3,7,9,13],[1,3,7,10,12],[1,3,7,10,13],[1,3,7,10,14],[1,3,7,11,13],[1,3,7,11,14],[1,4,6,9,12],[1,4,6,9,13],[1,4,6,10,12],[1,4,6,10,13],[1,4,6,10,14],[1,4,7,9,12],[1,4,7,9,13],[1,4,7,10,12],[1,4,7,10,13],[1,4,7,10,14],[1,4,7,11,13],[1,4,7,11,14],[1,4,8,10,12],[1,4,8,10,13],[1,4,8,10,14],[1,4,8,11,13],[1,4,8,11,14],[1,5,7,9,12],[1,5,7,9,13],[1,5,7,10,12],[1,5,7,10,13],[1,5,7,10,14],[1,5,7,11,13],[1,5,7,11,14],[1,5,8,10,12],[1,5,8,10,13],[1,5,8,10,14],[1,5,8,11,13],[1,5,8,11,14],[2,4,6,9,12],[2,4,6,9,13],[2,4,6,10,12],[2,4,6,10,13],[2,4,6,10,14],[2,4,7,9,12],[2,4,7,9,13],[2,4,7,10,12],[2,4,7,10,13],[2,4,7,10,14],[2,4,7,11,13],[2,4,7,11,14],[2,4,8,10,12],[2,4,8,10,13],[2,4,8,10,14],[2,4,8,11,13],[2,4,8,11,14],[2,5,7,9,12],[2,5,7,9,13],[2,5,7,10,12],[2,5,7,10,13],[2,5,7,10,14],[2,5,7,11,13],[2,5,7,11,14],[2,5,8,10,12],[2,5,8,10,13],[2,5,8,10,14],[2,5,8,11,13],[2,5,8,11,14]]
        ...
        def calculate_score(box):
            ...
            return int((score / 50) * bet)

        box = [r() for _ in range(15)]
        interaction = await ctx.respond(render(box))
        ...
        listener_task = asyncio.create_task(reaction_listener())
